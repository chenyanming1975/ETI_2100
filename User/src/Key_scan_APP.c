...
//-------------------------------------------------------------------------------------------
uint8_t Machine_note_use;					//´Ó»ú½Úµã
uint16_t Machine_note_temp;					//´Ó»ú½ÚµãÉèÖÃ

uint8_t	Key_code_bak;
uint8_t	Key_scan_time_cnt;                 //°´¼üÉ¨Ãè¶¨Ê±¼ÆÊı

volatile uint32_t E_wait_time_temp;				//2012-9-14 13:48
volatile uint32_t Prj_correlat;					//¹¤³ÌÏµÊıÊ¹ÓÃ2012-9-14 13:48

uint32_t Allow_error;						//Îó²îÔÊĞí·¶Î§2012-11-7 15:18
uint32_t Alarm_hight_nd;					//¸ßÅ¨¶È±¨¾¯Öµ2012-11-8 11:03
uint32_t Alarm_low_nd;						//µÍÅ¨¶È±¨¾¯Öµ2012-11-8 11:03
uint8_t	 Alarm_active_flag;					//±¨¾¯±êÖ¾2012-11-8 11:03

uint8_t	 Export_data_flag;					//2012-9-7 13:47

uint8_t  DCF_switch_select_flag;				//·§µÄ¿ª¹Ø±êÖ¾2012-7-18 15:04
...
uint8_t Menu_index_select;//ÔÚÖ÷²Ëµ¥ÖĞÑ¡ÔñÁËÁĞ±íÖĞµÄµÚ¼¸Ïî
uint8_t Menu_index_finc_select;//ÓÃÓÚ¼ÇÂ¼µÚ¶ş¼¾²Ëµ¥Ñ¡ÔñÁËµÚ¼¸Ïî
uint8_t Menu_lever3_select;//ÓÃÓÚ¼ÇÂ¼µÚÈı¼¶²Ëµ¥Ñ¡ÔñÁËµÚ¼¸Ïî

uint8_t Menu_lever1_point;//ÓÃÓÚµÚÒ»¼¶²Ëµ¥Ñ­»·ÏÔÊ¾
uint8_t Menu_lever2_point;//ÓÃÓÚµÚ¶ş¼¶²Ëµ¥µÄÑ­»·ÏÔÊ¾
uint8_t Menu_lever3_point;//ÓÃÓÚµÚÈı¼¶²Ëµ¥µÄÑ­»·ÏÔÊ¾


volatile uint8_t Menu_lever3_start_flag;			//2012-7-18 9:31

uint8_t Menu_lever3_PoPa;
uint8_t Menu_lever3_PoSeg; //ÔÚĞ¡ÇøÓò·´ÏÔÊ±,Ê¹ÓÃµÄ±äÁ¿

uint8_t Menu_lever4_PoPa;
uint8_t Menu_lever4_PoSeg;

uint8_t Menu_lever3_Pub_select_cnt;				//for position 2012-7-6 9:14
uint8_t Menu_lever3_Pub_adj_cnt;				//ÔÚÉèÖÃÊı×ÖÖµÊ±,Ê¹ÓÃ¸Ã±äÁ¿¼ÇÂ¼µ±Ç°ÊıÖµ
...
volatile uint8_t  	M_menur1_language_select_temp;		//ÓïÑÔÑ¡Ôñ:2-ÖĞÎÄ 1-Ó¢ÎÄ
...
volatile uint8_t  	M_menur1_dot_num_select_temp;		//Ğ¡ÊıµãÎ»ÊıÑ¡Ôñ
volatile uint8_t  	M_menur1_ZDXS_select_temp;		//×Ô¶¯Ï¡ÊÍÑ¡Ôñ
volatile uint8_t  	M_menur1_LZDJ_select_temp;		//Àë×Óµç×Ó 2012-7-10 16:35
volatile uint8_t  	M_menur1_DL_select_temp;		//¶¨Á¿·½·¨ 2012-7-10 16:36
volatile uint8_t  	M_menur1_direction_select_temp;		//½Á°è·½Ïò 2012-7-10 16:38
volatile uint16_t 	M_menur1_led_data_temp;			//²¨³¤
volatile uint32_t 	M_menur1_lange_data_temp;		//Á¿³ÌÑ¡Ôñ
volatile uint32_t 	M_menur1_led_dianliu_temp;		//LEDµçÁ÷ÉèÖÃ
volatile uint32_t 	M_menur1_JB_speed_temp;			//½Á°èËÙ¶È
volatile uint32_t 	M_menur1_ZSB_speed_temp;		//×¢Éä±ÃËÙ¶È
volatile uint32_t 	M_menur1_sample_v_temp;			//ÑùÆ·Ìå»ı

volatile uint32_t 	M_menur1_slope_temp;			//µ±Ç°Ê¹ÓÃµÄĞ±ÂÊ
volatile uint8_t	M_menur1_slope_flag;			//µ±Ç°Ê¹ÓÃµÄĞ±ÂÊ·ûºÅ
volatile uint8_t	Updata_slope_flag;			//µ±Ç°Ê¹ÓÃĞ±ÂÊÔÊĞí¸üĞÂ±êÖ¾

volatile uint8_t	Disple_flag;				//ÊÇ·ñÏû½â±êÖ¾

volatile uint32_t 	M_menur1_jz1nd_temp;			//±êÒº1Å¨¶È
volatile uint32_t 	M_menur1_jz1_v_temp;                    //±êÒº1Ìå»ı
volatile uint32_t 	M_menur1_jz2nd_temp;                    //±êÒº2Å¨¶È
volatile uint32_t 	M_menur1_jz2_v_temp;                    //±êÒº2Ìå»ı

volatile uint32_t 	M_menur1_sj1_v_temp;                    //ÊÔ¼Á1Ìå»ı
volatile uint32_t 	M_menur1_sj2_v_temp;                    //ÊÔ¼Á2Ìå»ı
volatile uint32_t 	M_menur1_sj3_v_temp;                    //ÊÔ¼Á3Ìå»ı
volatile uint32_t 	M_menur1_sj4_v_temp;                    //ÊÔ¼Á4Ìå»ı

volatile uint32_t  	M_menur1_FY_temp;			//Ğ£Õı·½³ÌµÄ³£Êì£¬±äÁ¿ÏµÊı£¬±äÁ¿ºÍ×Ô±äÁ¿
...
volatile uint8_t	 M_menur1_RX_time_temp;			//ÈóÏ´´ÎÊı
volatile uint32_t	 M_menur1_QYSJ_temp;            	//È¡ÑùÊ±¼ä
volatile uint32_t        M_menur1_CSSJ_temp;            	//²âÊÔÊ±¼ä
volatile uint32_t        M_menur1_XHPY_temp;            	//ĞÅºÅÆ¯ÒÆ
volatile uint32_t        M_menur1_DDSJ_temp;            	//µÈ´ıÊ±¼ä

volatile uint32_t        M_lever4_FXPL_temp;            	//·ÖÎöÆµÂÊ
volatile uint32_t        M_lever4_JZPL_temp;            	//Ğ£ÕıÆµÂÊ
volatile uint32_t        M_lever4_QXPL_temp;            	//ÇåÏ´ÆµÂÊ
volatile uint32_t        M_lever4_HCPL_temp;            	//ºË²éÆµÂÊ

volatile uint8_t	M_lever4_extern_trrig_flag;		//Íâ²¿´¥·¢±êÖ¾
...
volatile uint8_t	M_lever4_FXCS_temp;             	//·ÖÎö´ÎÊı
volatile uint32_t	M_lever4_BYND_temp;             	//±êÒºÅ¨¶È
volatile uint32_t	M_lever4_move_to_temp;          	//×¢Éä±ÃÒÆ¶¯µ½XXÎ»ÖÃ
volatile uint32_t	M_lever4_WD_temp;               	//ÎÂ¶ÈÖµ
volatile uint16_t	M_lever4_BCWD_temp;             	//²¹³¥ÎÂ¶ÈÖµ
volatile uint32_t	M_lever4_GDGDY_temp;            	//¹âµç¹ÜµçÑ¹
volatile uint32_t	M_lever4_PZDY_temp;             	//Æ«ÖÃµçÑ¹
...
uint32_t	M_lever4_tp_Out1_4ma_temp;                      //ÒÔÏÂÊÇÎÂ¶ÈµÄ4~20maÊä³öµÄ¼Ä´æÆ÷
...
uint32_t	M_lever4_jz_Out1_4ma_temp;                      //ÒÔÏÂÊÇĞ£Õı½á¹ûµÄ4~20maÊä³öµÄ¼Ä´æÆ÷
...
volatile uint32_t	M_lever3_HCPD_temp;			//ºË²éÅĞ¶Ï 2012-7-25 11:53 add
...
//===============================================================================
int32_t Shift_165_Data(void)			//°´¼üĞ¾Æ¬Çı¶¯
{
...
//===============================================================================
int32_t Read_IC_165_Data(void)			//¶ÁÇı¶¯Ğ¾Æ¬µÄ¼üÖµ
{
...
void Enter_EnterButton_disp(void)			
{
    ...
    else //¸ù¾İµ±Ç°½çÃæÉî¶È£¬µ÷ÕûMenu_lever_selectµÄÖµ
    {
        if((Menu_lever_select>=3) && (Key_code_bak != KEY_CODE_ESC))//Ã»ÓĞ°´ESC¼ü
        {
            if(Menu_lever_select==3)//µ±Ç°½çÃæÉî¶ÈÎª3
            {
                if((Menu_index_select==2)&&(Menu_index_finc_select==1))//Ö÷²Ëµ¥Ñ¡ÔñµÚ2ĞĞ-·ÖÎöÉèÖÃ£¬Ö÷²Ëµ¥µÄÏÂÒ»¼¶²Ëµ¥Ñ¡ÔñÁËµÚ1ĞĞ-·ÖÎöÎï
                    M_menur1_analy_obj_select_temp=Menu_lever3_Pub_select_cnt;//ÉèÖÃ·ÖÎö¶ÔÏó
                if((Menu_index_select==2)&&(Menu_index_finc_select==3))//Ö÷²Ëµ¥Ñ¡ÔñµÚ2ĞĞ-·ÖÎöÉèÖÃ£¬Ö÷²Ëµ¥µÄÏÂÒ»¼¶²Ëµ¥Ñ¡ÔñÁËµÚ3ĞĞ-µ¥Î»
                    M_menur1_unit_select_temp=Menu_lever3_Pub_select_cnt;//ÉèÖÃµ¥Î»
                if((Menu_index_select==2)&&(Menu_index_finc_select==11))//Ö÷²Ëµ¥Ñ¡ÔñµÚ2ĞĞ-·ÖÎöÉèÖÃ£¬Ö÷²Ëµ¥µÄÏÂÒ»¼¶²Ëµ¥Ñ¡ÔñÁËµÚ11ĞĞ-×Ô¶¯Ï¡ÊÍ
                    M_menur1_ZDXS_select_temp=Menu_lever3_Pub_select_cnt;//×Ô¶¯Ï¡ÊÍÑ¡Ôñ
                if((Menu_index_select==2)&&(Menu_index_finc_select==17))//Ö÷²Ëµ¥Ñ¡ÔñµÚ2ĞĞ-·ÖÎöÉèÖÃ£¬Ö÷²Ëµ¥µÄÏÂÒ»¼¶²Ëµ¥Ñ¡ÔñÁËµÚ17ĞĞ-Ğ±ÂÊ¸üĞÂ
                    Updata_slope_flag=Menu_lever3_Pub_select_cnt;       //µ±Ç°Ê¹ÓÃĞ±ÂÊÔÊĞí¸üĞÂ±êÖ¾

                if((Menu_index_select==2)&&(Menu_index_finc_select==18))//Ö÷²Ëµ¥Ñ¡ÔñµÚ2ĞĞ-·ÖÎöÉèÖÃ£¬Ö÷²Ëµ¥µÄÏÂÒ»¼¶²Ëµ¥Ñ¡ÔñÁËµÚ18ĞĞ-Ïû½â
                    Disple_flag=Menu_lever3_Pub_select_cnt;//ÉèÖÃÊÇ·ñÏû½â

                if((Menu_index_select==3)&&(Menu_index_finc_select==1))//Ö÷²Ëµ¥Ñ¡ÔñµÚ3ĞĞ-·½·¨ÉèÖÃ£¬Ö÷²Ëµ¥µÄÏÂÒ»¼¶²Ëµ¥Ñ¡ÔñÁËµÚ1ĞĞ-·ÖÎö·½·¨
                {
                    ...
                    {
                        M_menur1_slope_temp=0x00005960;//µ±Ç°Ê¹ÓÃµÄĞ±ÂÊ
                        M_menur1_slope_flag=2;         //µ±Ç°Ê¹ÓÃµÄĞ±ÂÊ·ûºÅ
                    }
                    else
                    {
                        M_menur1_slope_temp=0x00000100;//µ±Ç°Ê¹ÓÃµÄĞ±ÂÊ
                        M_menur1_slope_flag=1;         //µ±Ç°Ê¹ÓÃµÄĞ±ÂÊ·ûºÅ
                    }
                }
                ...
                if((Menu_index_select==3)&&(Menu_index_finc_select==8))//·½·¨ÉèÖÃ-Ğ¡ÊıµãÎ»Êı
                    M_menur1_dot_num_select_temp=Menu_lever3_Pub_select_cnt;//ÓĞ1£¬2£¬3¿ÉÑ¡
                if((Menu_index_select==3)&&(Menu_index_finc_select==13))//¹¤³ÌÏµÊı
                {
            ...
            else if(Menu_lever_select==4)//µ±Ç°½çÃæÉî¶ÈÎª4
            {
                ...
                if((Menu_index_select==6)&&(Menu_index_finc_select==1)&&(Menu_lever3_select==3))//ÊäÈëÊä³ö-RS485Êä³ö-ÆæÅ¼Ğ£Ñé
    ...
    switch(Menu_lever_select)//¸ù¾İµ±Ç°½çÃæÉî¶È,³õÊ¼»¯Ò»Ğ©±äÁ¿µÄÖµ
    {
    case 3:
        //clear lever4 ram
        Menu_lever4_Pub_select_cnt=1;//4¼¶²Ëµ¥Ñ¡Ôñ¼ÆÊı
...
//	edit		:sam 2012.06.14
//	modefy		:°´¼ü´¦Àíº¯Êı,ÔÚmainÖĞÑ­»·ÔËĞĞ
//===============================================================================
void Key_scan_prosecce(void)
{
    ...
    if(Key_scan_time_cnt ==0 )
    {
        Key_scan_time_cnt=0x6;			    //ÉèÖÃ°´¼üÉ¨Ãè¼ä¸ô Key_scan_bounce_timeº¯ÊıÖĞ×Ô¼õ

        Key_code_temp=Read_IC_165_Data() & 0x7f; 	//get key code (0-6bitÎªÓĞĞ§Öµ)
		
        if(Key_code_bak != Key_code_temp)
        {
            Key_code_bak = Key_code_temp;//½«±¾´Î°´¼üÖµ½øĞĞ±¸·İ
            switch(Key_code_temp)
            {
            case KEY_CODE_UP:
                Set_back_light_10sec();
                Set_en_buzz_finc(1,4,200,100);
                if(Prog_disp_flag!=1)			//×Ô¶¯·ÖÎöÖĞ´Ë°´¼üÎŞĞ§
                {
                    if(Check_pass_word_flag==1)		//ÊÇ·ñÊäÈëÃÜÂë
                    {
                        --Pass_adj_cnt;
                        if(Pass_adj_cnt>=10)
                            Pass_adj_cnt=9;

                        Pass_adj_disp(Pass_adj_cnt,Pass_bit_select);//¸üĞÂÃÜÂë½çÃæÏÔÊ¾µÄÊı×Ö
                    }
                    else
                    {
                        if(Menu_lever_select==0x01)		//µÚÒ»¼¶²Ëµ¥
                        {
                        ...
                        if(Menu_lever_select==0x02)		//µÚ¶ş¼¶²Ëµ¥
                        {
                            if(M_menur1_language_select_temp==2)  i=4;	//ÖĞÎÄ4ĞĞ
                            else i=6;      					//Ó¢ÎÄ6ĞĞ
                            switch(Menu_index_select)
                            {
                            case 0x01:
                        ...
                        if(Menu_lever_select==0x03)		//µÚÈı¼¶²Ëµ¥
                        {
...
//=============================================================================
            case KEY_CODE_LEFT:
                ...
                if(Prog_disp_flag==1)				//×Ô¶¯·ÖÎöÖĞ´Ë°´¼üÎŞĞ§
                    __nop();
                else if(Check_pass_word_flag==1)                   //ÊÇ·ñÊäÈëÃÜÂë
                {
                    if(Pass_bit_select>1)
                        --Pass_bit_select;
                    Pass_select_disp(Pass_bit_select);
                }
                else if(Menu_lever_select==0x03)		//µÚÈı¼¶²Ëµ¥
                {
                ...
                else if(Menu_lever_select==4)		//µÚËÄ¼¶²Ëµ¥
                {
...
//=============================================================================
            case KEY_CODE_ESC:
                ...
                {
//               	         DrvGPIO_SetBit (E_GPC,14);		//close alarm led£¨Çå³ı¸ß/µÍÅ¨¶È±¨¾¯£©
                ...
                i=Led_port_status;
                i&=BIT1;
                if((Prog_disp_flag==1)||(i==BIT1))			//ÈçÔÚ×Ô¶¯·ÖÎöÔòÇĞ»»µ½²Ëµ¥ÏÔÊ¾
                {
                    if(Prog_disp_flag==1)
                        Prog_disp_flag=2;
                ...
                else if(Pwm_menu_lever>1)			//ÈçÓĞ¶ÔPWMÊä³ö²Ù×÷ÔòÏÈÍË³ö¸Ã²Ù×÷
                {
                    --Pwm_menu_lever;
                    Disp_pwm_menu();
                ...
                else if(Menu_lever_select>0)				//Èç¹û²Ëµ¥´óÓÚ1¼¶Ôò¼ÌĞøÍË³ö²Ëµ¥
                {
                ...
                else if((Prog_disp_flag==0)&&(Menu_lever_select==0))	//ÈçÎŞ×Ô¶¯·ÖÎö²Ëµ¥Ñ¡Ôñ£¬ÏÔÊ¾×îºóÒ»´Î·ÖÎö½á¹û
                {
                ...
                else if((Prog_disp_flag==2)&&(Menu_lever_select==0))	//2012-9-1 22:42  ÈçÓĞ×Ô¶¯·ÖÎöÇĞ»»µ½×Ô¶¯·ÖÎöµÄ²Ëµ¥ÏÔÊ¾
                {
                    Prog_disp_flag=1;
                    LCD_disp_flag=1;
                    ...
                    if(M_menur1_LZDJ_select_temp==1)
                    {
                        ...
                        if((M_menur1_DL_select_temp==1)&&(Measurement_signal_22!=0))	//ÇúÏß·¨²»ÏÔÊ¾
                        {
...
//=============================================================================
            case KEY_CODE_ENTER:
                Set_back_light_10sec();
                Set_en_buzz_finc(1,4,200,100);
                if(Prog_disp_flag==1)//×Ô¶¯·ÖÎö¹ı³ÌÖĞ
                    break;

                if(Check_prio_finc())//¼ì²éÈ¨ÏŞ
                    break;
                else if(Menu_lever_select<3)	//²Ëµ¥Ğ¡ÓÚ3¼¶È·ÈÏ½øÈëÏÂÒ»¼¶²Ëµ¥
                {
                    ++Menu_lever_select;
                ...
                else if(Menu_lever_select==3)	//µÚÈı¼¶²Ëµ¥È·ÈÏÈë¿Ú
                {
                    if(Menu_lever3_start_flag>=0x10)
                    {
                        if(Wait_clock_flag==1) 
							__nop();			//2013-6-8 9:58 µÈ´ıµÄÊ±ºò¿ª·ÅĞŞ¸ÄÈ¨ÏŞ
                    ...
                    if(Display_lever4_en_flag())//ÅĞ¶ÏÊÇ·ñÓĞµÚËÄ¼¶²Ëµ¥
                    {
                        Menu_lever4_Pub_select_cnt=1;
                        Menu_lever4_Pub_adj_cnt=0;
                        Menu_lever_select=4;
                    ...
                    else if(Menu_index_select==3)			//updata program £¨Èí¼ş¸üĞÂ¿ØÖÆ£©
                    {
                ...
                {
                    if(Pwm_menu_lever>=1)			//2012-9-18 14:03 pwm menu£¨PWM²Ù×÷²Ëµ¥£©
                    {
                        ++Pwm_menu_lever;
                        if(Pwm_menu_lever==2)
                            Pwm_menu_select1=1;
                        ...
                        Menu_lever3_4_enter_finction();      //µÚËÄ¼¶²Ëµ¥È·ÈÏº¯Êı
...
//========================================================================================
            case KEY_CODE_POWER:
                Set_en_buzz_finc(1,4,200,100);
                if((Auto_check_clock_flag!=0)||(Menu_lever3_start_flag>=0x10))	//ÈçÔÚµÈ´ıÏÂ´Î·ÖÎöÔòÇå³ı¸ÃµÈ´ı
                {
...
////                     	 Hand_finction_select=0;
                ...
                if((Prog_disp_flag!=0)||(Hand_finction_select!=0))			//ÈçÔÚ×Ô¶¯·ÖÎöÍ£Ö¹×Ô¶¯·ÖÎö
                {
                    if(Prog_disp_flag==1)
                    {
                        if(Menu_lever_select==4)
                ...
                else if((Prog_disp_flag==0)&&(Hand_finction_select==0))		//Ö±½Ó»Øµ½ÏÔÊ¾×îºóÒ»´ÎÀúÊ·¼ÇÂ¼²Ëµ¥
                {
...
//========================================================================================
            case KEY_CODE_EXPORT:			//2012-9-7 13:51
                ...
                if(Menu_lever_select==0)			//Êı¾İµ¼³ö¿ØÖÆ
                {
...
/////                   }
    ...
    case 0x05: 					//²é¿´ÀúÊ·¼ÇÂ¼
        ...
        break;
//------------------------------------------------
    case 0x01: 					//¿ªÊ¼¸÷ÖÖ·ÖÎöÉèÖÃ-²ÎÊıµ÷Õû
        if(M_menur1_language_select_temp==2)  i=4;
        else i=6;
        switch(Menu_index_finc_select)
        {
...
////                      Menu_lever3_start_flag&=~BIT3;
        ...
        break;
    case 0x06: 					//ÊäÈëÊä³öÉèÖÃ-²ÎÊıµ÷Õû
        if(M_menur1_language_select_temp==2)  i=4;
        else i=6;
        switch(Menu_index_finc_select)
        {
...
//---------------------------------------------
    case 4:					//ÊÖ¹¤²Ù×÷-²ÎÊıµ÷Õû
    case 1: 					//¿ªÊ¼¸÷ÖÖ·ÖÎöÉèÖÃ-²ÎÊıµ÷Õû
    case 6: 					//ÊäÈëÊä³öÉèÖÃ-²ÎÊıµ÷Õû
        if(Menu_lever3_point>1)
        {
            --Menu_lever3_point;
            --Menu_lever3_select;
...
/////   if(Menu_lever_select==3)
/////      return_disp_flag=0;
    if(Menu_lever3_start_flag>=0x10)		//×Ô¶¯·ÖÎöµÄÊ±ºòÆÁ±Î²Ëµ¥1,4,6
    {
    ...
    case 0x01:		//¿ªÊ¼
        ...
        if(Menu_lever_select==3)//3¼¶²Ëµ¥
        {
            ...
            case 3:
                ...
                {
                    Menu_lever3_start_flag&=~BIT2;
                    Menu_lever3_start_flag=BIT6;//ÊÖ¶¯Ğ£Õı
                    Memu1_auto_step_status=0;
...
//            	     	     Set_JB_direction(); 		//moto on
            ...
            break;
        case 6:								//Àë×Óµç¼«Ñ¡Ôñ
            if((Menu_lever3_select==2)&&(Menu_lever_select==4))		//¶Ë¿ÚĞ£×¼
            {
...
#else
                if(M_menur1_LZDJ_select_temp==1)
                    Set_tx_cmd_data_finc(U_SENCER_MODU_ADDS,0x06,U_SENSER_NTC_CTRL_WATER_ADDS,k,0x00);		//µç¼«·¨  2013-6-7 10:58
                else
                    Set_tx_cmd_data_finc(U_SENCER_MODU_ADDS,0x06,U_SENSER_NTC_CTRL_ADDS,k,0x00); 		//±ÈÉ«·¨
#endif
            }
            break;
        case 8:
            if((Menu_lever3_select==1)&&(Menu_lever_select==4))				//LEDµçÁ÷
            {
                j=k=0;
                i=M_menur1_led_dianliu_temp>>16;
                i&=0x0f;
            ...
            else if((Menu_lever3_select==3)&&(Menu_lever_select==4))		//Æ«ÖÃµçÑ¹
            {
                j=k=0;
...
//	modefy		:null
//===============================================================================
void Lever4_Select_finc_adds(void)			//×óÓÒÑ¡ÔñÒªÉèÖÃµÄÑ¡Ïî²¢ÏÔÊ¾
{
    uint8_t i;
...
//	edit		:sam 2012-8-13 17:03
//	modefy		:ÔÚ½çÃæÉèÖÃ²¨ÌØÂÊºóÖØĞÂ³õÊ¼»¯´®¿Ú2
//===============================================================================
